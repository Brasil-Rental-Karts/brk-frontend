...
export class PDFGenerator {
   ...
   */
  private static async generateCoverPage(
    pdf: jsPDF,
   ...
   */
  private static async generateContentPages(
    pdf: jsPDF,
    data: RegulationPDFData,
  ): Promise<void> {
    const pageWidth = pdf.internal.pageSize.getWidth();
    ...
    data.regulations.forEach((regulation, index) => {
      ...
      currentY = this.renderMarkdownTitle(
        pdf,
        `${index + 1}. ${cleanTitle}`,
        margin,
        currentY,
      ...
      currentY = this.renderMarkdownContent(
        pdf,
        regulation.content,
        margin,
        currentY,
    ...
    blocks.forEach((block) => {
      ...
      switch (block.type) {
        ...
        case "table":
          ...
          break;
        case "blockquote":
          currentY = this.renderBlockquote(
            pdf,
            block.content,
            x,
            currentY,
  ...
  private static renderTable(
    ...
    const cellPadding = 2;
    const tableHeight =
      table.filter((row) => !row.every((cell) => /^:?-{3,}:?$/.test(cell)))
        .length *
        rowHeight +
      4;
   ...
   */
  private static parseMarkdownText(
    text: string,
   ...
   */
  private static parseMarkdownBlocks(content: string): Array<{
    type: string;
    content: string;
    items?: string[];
    level?: number;
    ...
    lines.forEach((line) => {
      ...
      if (trimmedLine) {
        ...
        if (currentBlock.content && currentBlock.content.trim()) {
          ...
          const needsSpace =
            !/[.!?]/.test(lastChar) && !/\s$/.test(currentBlock.content);
