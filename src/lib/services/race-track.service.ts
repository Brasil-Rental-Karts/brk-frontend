...
export const RaceTrackService = {
  ...
  getByName: async (name: string): Promise<RaceTrack | null> => {
    try {
      const response = await api.get<RaceTrackResponse>(
        `/race-tracks/name/${name}`,
      );
      return response.data.data;
    } catch (error: any) {
  ...
  getByCity: async (city: string): Promise<RaceTrack[]> => {
    const response = await api.get<RaceTracksResponse>(
      `/race-tracks/city/${city}`,
    );
    return response.data.data;
  },
  ...
  update: async (
    id: string,
    data: UpdateRaceTrackRequest,
  ): Promise<RaceTrack> => {
    const response = await api.put<RaceTrackResponse>(
      `/race-tracks/${id}`,
      data,
    );
    return response.data.data;
  ...
  toggleActive: async (id: string): Promise<RaceTrack> => {
    const response = await api.patch<RaceTrackResponse>(
      `/race-tracks/${id}/toggle-active`,
    );
    return response.data.data;
  },
};
