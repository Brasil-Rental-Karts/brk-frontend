...
export class LapTimesService {
  ...
  static async getLapTimesByStageAndCategory(stageId: string, categoryId: string): Promise<LapTimes[]> {
    const response = await api.get(`${this.baseUrl}/stage/${stageId}/category/${categoryId}`);
    return response.data;
  }
  ...
  static async getLapTimesByUser(
    ...
    const response = await api.get(`${this.baseUrl}/stage/${stageId}/category/${categoryId}/user/${userId}`, { params });
    return response.data;
  }
  ...
  static async saveLapTimes(
    ...
    const response = await api.post(`${this.baseUrl}/stage/${stageId}/category/${categoryId}/user/${userId}`, data);
    return response.data;
  }
  ...
  static async importLapTimesFromExcel(
    ...
    const response = await api.post(`${this.baseUrl}/stage/${stageId}/category/${categoryId}/import`, data);
    return response.data;
  }
  ...
  static processExcelData(data: any[]): {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (row && row.length > 0) {
        ...
        const hasVLT = row.some((cell: any) => String(cell).trim().toUpperCase() === 'VLT');
        const hasTV = row.some((cell: any) => String(cell).trim().toUpperCase() === 'TV');
