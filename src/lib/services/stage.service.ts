...
export type { CreateStageData, Stage, StageStats, UpdateStageData };
...
export class StageService {
  ...
  static async getBySeasonId(seasonId: string): Promise<Stage[]> {
    try {
      const response = await api.get<Stage[]>(
        `${StageService.BASE_URL}/season/${seasonId}`,
      );
      return response.data;
    } catch (error: any) {
  ...
  static async getUpcomingBySeasonId(seasonId: string): Promise<Stage[]> {
    const response = await api.get<Stage[]>(
      `${StageService.BASE_URL}/season/${seasonId}/upcoming`,
    );
    return response.data;
  }
  ...
  static async getPastBySeasonId(seasonId: string): Promise<Stage[]> {
    const response = await api.get<Stage[]>(
      `${StageService.BASE_URL}/season/${seasonId}/past`,
    );
    return response.data;
  }
  ...
  static async getNextBySeasonId(seasonId: string): Promise<Stage | null> {
    try {
      const response = await api.get<Stage>(
        `${StageService.BASE_URL}/season/${seasonId}/next`,
      );
      return response.data;
    } catch (error: any) {
  ...
  static validateStageData(data: CreateStageData | UpdateStageData): string[] {
    ...
    if (
      "time" in data &&
      data.time &&
      !/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/.test(data.time)
    ) {
    ...
    if (
      "raceTrackId" in data &&
      (!data.raceTrackId || data.raceTrackId.trim().length === 0)
    ) {
  ...
  static async updateSchedule(id: string, schedule: any): Promise<Stage> {
    const response = await api.put<Stage>(
      `${StageService.BASE_URL}/${id}/schedule`,
      { schedule },
    );
    return response.data;
  }
   ...
   */
  static async saveKartDrawAssignments(
    stageId: string,
    assignments: any,
  ): Promise<any> {
    const response = await api.patch(
      `${this.BASE_URL}/${stageId}/kart-draw`,
      assignments,
    );
    return response.data;
  }
  ...
  static async saveStageResults(stageId: string, results: any): Promise<any> {
    const response = await api.patch(
      `${this.BASE_URL}/${stageId}/stage-results`,
      results,
    );
    return response.data.data;
  }
