...
export const useDashboardChampionships =
  (): UseDashboardChampionshipsReturn => {
    const [loadingChampionships, setLoadingChampionships] = useState(true);
    const [championshipsError, setChampionshipsError] = useState<string | null>(
      null,
    );
    ...
    const fetchMyChampionships = async () => {
      try {
        ...
        const filteredChampionships = allChampionships.filter(
          (championship) => championship.isOwner || championship.isStaff,
        );
