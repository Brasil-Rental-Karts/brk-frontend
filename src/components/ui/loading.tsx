...
export type LoadingProps = SpinnerLoadingProps | FullPageLoadingProps;
...
const SpinnerLoading = ({
  ...
  return (
    ...
    >
      <Loader2
        className={cn(config.spinner, "animate-spin", variantConfig[variant])}
      />
      {message && (
...
export const DataLoader = ({
  message = defaultMessages.data,
}: {
  message?: string;
}) => <Loading type="spinner" size="lg" message={message} />;

export const FormLoader = ({
  message = defaultMessages.form,
}: {
  message?: string;
}) => <Loading type="spinner" size="md" message={message} />;

export const SaveLoader = ({
  message = defaultMessages.save,
}: {
  message?: string;
}) => <Loading type="spinner" size="md" message={message} />;

export const DeleteLoader = ({
  message = defaultMessages.delete,
}: {
  message?: string;
}) => <Loading type="spinner" size="md" message={message} />;

export const UploadLoader = ({
  message = defaultMessages.upload,
}: {
  message?: string;
}) => <Loading type="spinner" size="md" message={message} />;

export const SearchLoader = ({
  message = defaultMessages.search,
}: {
  message?: string;
...
export const ButtonLoader = ({ size = "sm" }: { size?: LoadingSize }) => (
  <Loader2
    className={cn(sizeConfig[size].spinner, "animate-spin text-current")}
  />
);
