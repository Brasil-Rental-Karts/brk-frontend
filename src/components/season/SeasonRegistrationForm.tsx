...
import { Stage, StageService } from "@/lib/services/stage.service";
...
const CategoryOptionBadge = React.forwardRef<
  HTMLDivElement,
  {
    category: Category;
    checked: boolean;
...
const StageOptionBadge = React.forwardRef<
  HTMLDivElement,
  {
    stage: Stage;
    checked: boolean;
...
const CategorySelectionComponent = React.forwardRef<
  HTMLDivElement,
  {
    value?: string[];
...
>(
  (
    { value = [], onChange, disabled, categories, categoryRegistrationCounts },
    ref,
  ) => {
...
const StageSelectionComponent = React.forwardRef<
  HTMLDivElement,
  {
    value?: string[];
    onChange: (value: string[]) => void;
...
export const SeasonRegistrationForm: React.FC<SeasonRegistrationFormProps> = ({
  ...
  const inscriptionType =
    selectedCondition ||
    (season ? SeasonService.getInscriptionType(season) : "por_temporada");
  ...
  const filterStages = useCallback(
    (allStages: Stage[], registrations: SeasonRegistration[]) => {
      ...
      if (
        ...
        userRegistration &&
        userRegistration.stages
      ) {
        userRegisteredStageIds = userRegistration.stages.map(
          (stage) => stage.stageId,
        );
      }
    ...
    try {
      ...
      await Promise.all(
        categoriesData.map(async (category: Category) => {
          try {
            const count =
              await SeasonRegistrationService.getCategoryRegistrationCount(
                category.id,
              );
            counts[category.id] = count;
          } catch (error) {
            console.error(
              `Erro ao buscar contagem para categoria ${category.id}:`,
              error,
            );
      ...
      if (seasonData.inscriptionType === "por_etapa") {
        if (stagesData.length === 0) {
          console.warn(
            "⚠️ [FRONTEND] Temporada é por etapa mas não há etapas cadastradas!",
          );
        }
      }
    } catch (err: any) {
      ...
      await Promise.all(
        installmentsToCheck.map(async (installments) => {
          try {
            ...
            console.error(
              `Erro ao buscar taxa para ${installments} parcelas:`,
              error,
            );
  ...
  useEffect(() => {
    if (selectedPaymentMethod === "cartao_credito") {
      const totalWithConfigurableFees = calculateTotalWithConfigurableFees(
        total,
  ...
  const getCurrentTotalWithFees = useCallback(() => {
    if (selectedPaymentMethod === "cartao_credito") {
      return calculateTotalWithConfigurableFees(
        total,
        selectedPaymentMethod,
        selectedInstallments,
      );
  ...
  const getMaxInstallments = (paymentMethod: string) => {
    ...
    switch (paymentMethod) {
      case "pix":
        return SeasonService.getPixInstallmentsForCondition(
          season,
          inscriptionType,
        );
      case "cartao_credito":
        return SeasonService.getCreditCardInstallmentsForCondition(
          season,
          inscriptionType,
        );
      default:
  ...
  const getConfigurableRate = (installments: number) => {
    ...
    throw new Error(
      "Taxa de cartão de crédito não configurada para este campeonato e número de parcelas. Solicite ao administrador que configure as taxas no painel.",
    );
  };
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Erro ao calcular taxa de cartão de crédito.",
      );
      return baseTotal;
    }
  };
  ...
  const generateInstallmentOptions = (paymentMethod: string) => {
    ...
    for (let i = 1; i <= maxInstallments; i++) {
      if (paymentMethod === "pix") {
        ...
        try {
          const totalWithFees = calculateTotalWithConfigurableFees(
            total,
            paymentMethod,
            i,
          );
  ...
  useEffect(() => {
    ...
    const availablePaymentMethods = SeasonService.getPaymentMethodsForCondition(
      season,
      inscriptionType,
    );
    if (availablePaymentMethods.length === 0) {
      ...
      setError(
        "Esta temporada não possui métodos de pagamento configurados. Entre em contato com os organizadores.",
      );
      return;
    }
  ...
  const getInscriptionValue = () => {
    ...
    if (selectedCondition && season.paymentConditions) {
      const condition = season.paymentConditions.find(
        (c) => c.type === selectedCondition && c.enabled,
      );
      return condition
        ? condition.value
        : SeasonService.getInscriptionValue(season);
    }
          ...
          <CardContent className="p-6">
            <div className="text-center">
              ...
              <p className="text-muted-foreground mb-4">
                ...
                sobre quando as inscrições serão abertas.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-left bg-gray-50 p-4 rounded-lg">
                <div>
                  <strong>Valor por categoria:</strong>{" "}
                  {formatCurrency(getInscriptionValue())}
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-left bg-gray-50 p-4 rounded-lg">
                <div>
                  ...
                  <span className="capitalize">
                    {translateRegistrationStatus(userRegistration.status)}
                  </span>
                </div>
                <div>
                  <strong>Status do pagamento:</strong>{" "}
                  <span className="capitalize">
                    {translatePaymentStatus(userRegistration.paymentStatus)}
                  </span>
                </div>
                <div>
                  <strong>Valor pago:</strong>{" "}
                  {formatCurrency(userRegistration.amount)}
                </div>
                <div>
  ...
  if (
    ...
    if (futureStages.length === 0) {
      ...
      description =
        "Você já se inscreveu em todas as etapas futuras desta temporada.";
        ...
        <Card>
          ...
          <CardContent className="p-6">
            <div className="text-center">
              <div className="mb-4">
                <div
                  className={`inline-flex items-center px-4 py-2 rounded-full ${badgeColor} font-medium`}
                >
                  <svg
                    className="w-5 h-5 mr-2"
                    fill="currentColor"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-left bg-gray-50 p-4 rounded-lg">
                ...
                <div>
                  <strong>Etapas passadas:</strong>{" "}
                  {stages.length - futureStages.length}
                </div>
                <div>
                  <strong>Valor por categoria:</strong>{" "}
                  {formatCurrency(getInscriptionValue())}
                  ...
                  {championship &&
                    !championship.commissionAbsorbedByChampionship && (
                      <div className="text-xs text-gray-500">
                        ...
                        {Math.round(
                          championship.platformCommissionPercentage || 10,
                        )}
                        % incluído
                      </div>
                    )}
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex space-x-3">
                      {SeasonService.getPaymentMethodsForCondition(
                        season,
                        inscriptionType,
                      ).includes("pix") && (
                        <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded">
                          PIX até{" "}
                          {SeasonService.getPixInstallmentsForCondition(
                            season,
                            inscriptionType,
                          )}
                          x
                      ...
                      {SeasonService.getPaymentMethodsForCondition(
                        season,
                        inscriptionType,
                      ).includes("cartao_credito") && (
                        <span className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">
                          Cartão até{" "}
                          {SeasonService.getCreditCardInstallmentsForCondition(
                            season,
                            inscriptionType,
                          )}
                          x
        ...
        {formConfig.length > 0 ? (
          ...
          <div className="text-center p-8">
            ...
            <p className="text-sm text-gray-500 mt-2">
              formConfig.length: {formConfig.length}
            </p>
          </div>
        )}
        ...
        >
          <DialogContent className="w-[95vw] max-w-md p-4 sm:p-6">
            <DialogHeader>
              ...
              <DialogDescription>
                Você tem uma inscrição em andamento. Tem certeza que deseja sair
                sem finalizar a inscrição?
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
        ...
        <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4 text-center">
            ...
            <p className="text-gray-600 mb-4">
              Sua inscrição está sendo processada. Por favor, aguarde e não
              feche esta tela.
            </p>
