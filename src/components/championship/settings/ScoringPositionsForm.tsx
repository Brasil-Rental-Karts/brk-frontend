...
export const ScoringPositionsForm = forwardRef<HTMLDivElement, ScoringPositionsFormProps>(
  ({ value = [], onChange, disabled = false }, ref) => {
    ...
    useEffect(() => {
      if (positions.length > previousPositionsCount && positionsContainerRef.current) {
        setTimeout(() => {
    ...
    const updatePosition = (index: number, field: 'position' | 'points', value: number) => {
            ...
            {positions.map((position, index) => (
              <div key={index} className="flex flex-col space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => updatePosition(index, 'points', parseInt(e.target.value) || 0)}
                  placeholder="0"
                  disabled={disabled}
