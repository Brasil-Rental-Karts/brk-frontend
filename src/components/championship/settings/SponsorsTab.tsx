...
export const SponsorsTab = ({ championshipId }: SponsorsTabProps) => {
  ...
  const saveSponsors = useCallback(
    async (sponsors: any[]) => {
      setIsSaving(true);
      setError(null);
      ...
      try {
        ...
        const message =
          err.message || "Erro ao salvar patrocinadores e apoiadores";
        setError(message);
        toast.error(message);
      } finally {
        setIsSaving(false);
  ...
  const { isLoading, initialData, onFormReady } = useFormScreen<
    Championship,
    { sponsors: any[] }
  >({
    id: championshipId,
  ...
  const handleSponsorsChange = useCallback(
    (sponsors: any[]) => {
      ...
      const hasChanged =
        JSON.stringify(currentSponsors) !== JSON.stringify(sponsors);
    ...
    return (
      <div className="flex items-center justify-center py-8">
        <Loading
          type="spinner"
          size="sm"
          message="Carregando patrocinadores e apoiadores..."
        />
      </div>
    );
  }
