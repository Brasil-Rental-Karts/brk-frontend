...
import { Button, Badge, Card, CardContent, Tabs, TabsContent, TabsList, TabsTrigger } from 'brk-design-system';
import { Check, X, Users, UserCheck, UserX, Loader2, AlertTriangle, CheckCircle, Clock, AlertCircle } from 'lucide-react';
import { StageParticipationService, BulkConfirmParticipationData, BulkCancelParticipationData } from '@/lib/services/stage-participation.service';
...
export const BulkConfirmPilotsModal: React.FC<BulkConfirmPilotsModalProps> = ({ 
  ...
  const loadPilots = () => {
    try {
      ...
      const selectedStage = allStages.find((stage: any) => stage.id === stageId);
      if (!selectedStage) {
      ...
      const stageRegistrations = allRegistrations.filter(reg => 
        reg.season.id === selectedStage.seasonId
      );
      ...
      stageCategories.forEach(category => {
        ...
        categoryPilots.forEach(registration => {
          const isConfirmed = stageParticipations.some(
            (part) => part.userId === registration.userId && 
  ...
  const selectAll = () => {
    ...
    setPilots(prev => prev.map(pilot => ({
      ...pilot,
      isSelected: filteredPilots.some(fp => fp.userId === pilot.userId && fp.categoryId === pilot.categoryId)
    })));
  };
  ...
  const handleSave = async () => {
    ...
    if (selectedPilots.length === 0) {
      toast.error(`Selecione pelo menos um piloto para ${activeTab === 'confirm' ? 'confirmar' : 'cancelar'}`);
      return;
    }
    ...
    try {
      ...
      if (activeTab === 'confirm') {
        ...
        const response = await StageParticipationService.bulkConfirmParticipation(data);

        if (response.errors.length > 0) {
          toast.warning(`${response.data.length} pilotos confirmados com sucesso, ${response.errors.length} com erro`);
        } else {
          toast.success(`${response.data.length} pilotos confirmados com sucesso!`);
        }
      } else {
        ...
        const response = await StageParticipationService.bulkCancelParticipation(data);

        if (response.errors.length > 0) {
          toast.warning(`${response.data.length} pilotos cancelados com sucesso, ${response.errors.length} com erro`);
        } else {
          toast.success(`${response.data.length} pilotos cancelados com sucesso!`);
        }
      }
      ...
      toast.error(error.message || `Erro ao ${activeTab === 'confirm' ? 'confirmar' : 'cancelar'} pilotos`);
    } finally {
      setSaving(false);
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          <Tabs value={activeTab} onValueChange={(value) => !saving && setActiveTab(value as 'confirm' | 'cancel')}>
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="confirm" className="flex items-center gap-2" disabled={saving}>
              <Check className="h-4 w-4" />
              Confirmar Participação
            </TabsTrigger>
            <TabsTrigger value="cancel" className="flex items-center gap-2" disabled={saving}>
              <X className="h-4 w-4" />
          ...
          <TabsContent value="confirm" className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4 p-4 bg-muted rounded-lg">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-gray-600">{filteredPilots.length}</div>
            ...
            {/* Lista de pilotos */}
            {loading ? (
              <Loading type="spinner" size="md" message="Carregando pilotos..." />
            ) : error ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredPilots.map((pilot) => (
                  <Card 
                    ...
                    onClick={() => !saving && togglePilotSelection(pilot.userId, pilot.categoryId)}
                  >
          ...
          <TabsContent value="cancel" className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4 p-4 bg-muted rounded-lg">
              <div className="text-center">
                <div className="text-2xl font-bold text-red-600">{selectedCount}</div>
                <div className="text-sm text-muted-foreground">Selecionados</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-green-600">{filteredPilots.length}</div>
                <div className="text-sm text-muted-foreground">
              ...
              <>
                {activeTab === 'confirm' ? (
                  <>
                    ...
                    Confirmar {selectedCount} Piloto{selectedCount !== 1 ? 's' : ''}
                  </>
                ) : (
                  <>
                    <X className="h-4 w-4 mr-2" />
                    Cancelar {selectedCount} Piloto{selectedCount !== 1 ? 's' : ''}
                  </>
                )}
              </>
            )}
