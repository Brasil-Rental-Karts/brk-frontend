...
import { StageParticipation } from "@/lib/services/stage-participation.service";
...
const RaceDayHeader: React.FC<{
  ...
  <div className="flex items-center justify-between mb-6">
        ...
        <div className="relative">
          <select
            ...
            onChange={(e) => {
              ...
              const newSeasonStages = stages.filter(
                (stage) => stage.seasonId === seasonId,
              );
    ...
    <Button
      variant="outline"
      className="rounded-full px-4 py-1 h-8 text-sm font-semibold flex items-center gap-1"
    >
      Visão geral
...
export const RaceDayTab: React.FC<RaceDayTabProps> = ({ championshipId }) => {
  ...
  function getClosestStage(stages: any[]) {
    ...
    const stagesWithDate = stages.map((s) => ({
      ...
      dateStr: s.date ? s.date.slice(0, 10) : null,
    }));
    // Filtrar etapas de hoje ou futuras
    const futureOrTodayStages = stagesWithDate
      .filter((s) => {
  ...
  const selectedSeason =
    seasons.find((s: SeasonWithStages) => s.id === selectedSeasonId) ||
    seasons[0];
  ...
  const stages = useMemo(
    () =>
      allContextStages.filter(
        (stage: StageType) => stage.seasonId === selectedSeasonId,
      ),
  ...
  const categories = contextCategories.filter(
    (cat) => cat.seasonId === selectedSeasonId,
  );
  const registrations = contextRegistrations.filter(
    (reg) => reg.season.id === selectedSeasonId,
  );
  const filteredPenalties = contextPenalties.filter(
    (penalty) => penalty.stageId === selectedStageId,
  );

  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [stageParticipations, setStageParticipations] = useState<
    StageParticipation[]
  >([]);
  ...
  const [fleetDrawResults, setFleetDrawResults] = useState<{
    [categoryId: string]: {
      [pilotId: string]: { [batteryIndex: number]: { kart: number } };
    };
  }>({});
  ...
  const [selectedPilotForKartChange, setSelectedPilotForKartChange] = useState<{
    categoryId: string;
    pilotId: string;
    batteryIndex: number;
  } | null>(null);
  ...
  const [showPositionSelectionModal, setShowPositionSelectionModal] =
    useState(false);
  ...
  const [selectedPilotsForChart, setSelectedPilotsForChart] = useState<
    string[]
  >([]);
  ...
  const performFleetDraw = () => {
    ...
    categories.forEach((category) => {
      ...
      categoryPilots.forEach((pilot) =>
        pilotKartHistory.set(pilot.userId, new Set()),
      );
      ...
      for (
        ...
        const pilotsForThisBattery = [...categoryPilots];
        // Shuffle
        ...
        while (!success && attempts < 20) {
          ...
          for (let i = 0; i < pilotsForThisBattery.length; i++) {
            ...
            const possibleKarts = kartsForThisBattery.filter(
              (kart) => !history.has(kart) && !usedKarts.has(kart),
            );
            ...
            // Sorteia um kart
            const kart =
              possibleKarts[Math.floor(Math.random() * possibleKarts.length)];
            usedKarts.add(kart);
            // Salva resultado
            if (!results[category.id][pilot.userId])
              results[category.id][pilot.userId] = {};
  ...
  const loadFleets = async () => {
    ...
    try {
      ...
      if (
        selectedStage.fleets &&
        Array.isArray(selectedStage.fleets) &&
        selectedStage.fleets.length > 0
      ) {
        ...
        if (selectedStage.raceTrackId) {
          try {
            ...
            if (
              raceTrack?.defaultFleets &&
              Array.isArray(raceTrack.defaultFleets) &&
              raceTrack.defaultFleets.length > 0
            ) {
              // Converter frotas padrão do kartódromo para o formato esperado
              const defaultFleets = raceTrack.defaultFleets.map(
                (fleet: any, index: number) => ({
                  id: `default-${index + 1}`,
                  name: fleet.name,
            ...
            console.error(
              "Erro ao buscar frotas padrão do kartódromo:",
              raceTrackError,
            );
  ...
  const handleFleetKartsChange = (id: string, totalKarts: number) => {
    setFleets((prevFleets) => {
      ...
      if (wouldBeActive < minRequired) {
        toast.error(
          `Não é possível reduzir. Mínimo de ${minRequired} karts ativos é necessário.`,
        );
        return prevFleets;
      }
      const newFleets = prevFleets.map((f) =>
        f.id === id ? { ...f, totalKarts } : f,
      );
      saveFleets(newFleets);
      return newFleets;
    });
  ...
  const loadSchedule = async () => {
    ...
    try {
      ...
      const contextStage = allContextStages.find(
  ...
  const addScheduleItem = () => {
    ...
    const insertIndex = scheduleItems.findIndex(
  ...
  const handleDuplicateItem = (item: ScheduleItem) => {
    ...
    // Encontrar a posição correta baseada no horário
    const insertIndex = scheduleItems.findIndex(
      (scheduleItem) => scheduleItem.time > newItem.time,
    );
    const finalIndex = insertIndex === -1 ? scheduleItems.length : insertIndex;
  ...
  const SortableItem = ({ item }: { item: ScheduleItem }) => {
    ...
    const [localEditForm, setLocalEditForm] = React.useState({
      label: item.label,
      time: item.time,
    });
    React.useEffect(() => {
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (
                        !localEditForm.label.trim() ||
                        !localEditForm.time.trim()
                      )
                        return;
                      ...
                      const updatedSchedule = scheduleItems.map((it) =>
                        it.id === item.id
                          ? {
                              ...it,
                              label: localEditForm.label.trim(),
                              time: localEditForm.time.trim(),
                            }
                          : it,
                      );
                      setScheduleItems(updatedSchedule);
                    ...
                    disabled={
                      !localEditForm.label.trim() || !localEditForm.time.trim()
                    }
                    className="flex-1"
                  >
                    Salvar
  ...
  const [inactiveKarts, setInactiveKarts] = useState<{
  ...
  const toggleKartActive = (fleetId: string, kartIdx: number) => {
    setInactiveKarts((prev) => {
      ...
      if (current.includes(kartIdx)) {
        ...
        if (wouldBeActive < minRequired) {
          toast.error(
            `Não é possível inativar este kart. Mínimo de ${minRequired} karts ativos é necessário.`,
          );
          return prev;
        }
  ...
  useEffect(() => {
    if (selectedStageId) {
      const selectedStage = allContextStages.find(
  ...
  const handleSaveFleetDraw = async (results?: any) => {
    ...
    await StageService.saveKartDrawAssignments(
      selectedStageId,
      dataWithFleetAssignments,
    );
    ...
    setCategoryFleetAssignments(
      dataWithFleetAssignments.categoryFleetAssignments,
    );
  ...
  const changePilotKart = async (newKart: number) => {
    ...
    const currentKart =
      fleetDrawResults[categoryId]?.[pilotId]?.[batteryIndex]?.kart;

    // Verificar se o novo kart já está sendo usado por outro piloto
    const pilotUsingKart = Object.entries(
      fleetDrawResults[categoryId] || {},
    ).find(([otherPilotId, batteryResults]) => {
      return (
        otherPilotId !== pilotId &&
        batteryResults[batteryIndex]?.kart === newKart
      );
    });
    ...
    try {
      ...
      if (pilotUsingKart) {
        toast.success(
          `Kart ${newKart} transferido do piloto anterior para ${formatName(registrations.find((r) => r.userId === pilotId)?.user?.name || pilotId)}!`,
        );
      } else {
  ...
  const getAvailableKarts = (
    ...
    const allActiveKarts = Array.from(
      { length: fleet.totalKarts },
      (_, i) => i + 1,
    ).filter((kart) => !inactiveKartsForFleet.includes(kart - 1));
    ...
    Object.entries(fleetDrawResults[categoryId] || {}).forEach(
      ([pilotId, batteryResults]) => {
  ...
  const hasBlockingStatus = (
    ...
    pilotId: string,
    batteryIndex: number,
    positionType: "startPosition" | "finishPosition",
  ) => {
  ...
  const openPositionSelectionModal = (
    ...
    if (hasBlockingStatus(categoryId, pilotId, batteryIndex, type)) {
      ...
      toast.error(
        `Não é possível alterar a posição de corrida de um piloto com status ${statusText}`,
      );
      return;
    }
  ...
  const changePilotPosition = async (position: number) => {
    ...
    const { categoryId, pilotId, batteryIndex, type } =
  ...
  const clearPilotPosition = async () => {
    ...
    try {
      ...
      closePositionSelectionModal();
      toast.success(
        `${type === "startPosition" ? "Posição de qualificação" : "Posição de corrida"} removida!`,
      );
    } catch (error) {
  ...
  const [showQualifyingBestLapModal, setShowQualifyingBestLapModal] =
    useState(false);
  const [
    selectedPilotForQualifyingBestLap,
    setSelectedPilotForQualifyingBestLap,
  ] = useState<{
    categoryId: string;
    pilotId: string;
    batteryIndex: number;
  } | null>(null);
  const [qualifyingBestLapTimeInput, setQualifyingBestLapTimeInput] =
    useState("");
  const [qualifyingBestLapLoading, setQualifyingBestLapLoading] =
    useState(false);
  ...
  const openBestLapModal = (
    ...
    const currentBestLap =
      stageResults[categoryId]?.[pilotId]?.[batteryIndex]?.bestLap || "";
  ...
  const saveBestLap = async () => {
    ...
    updatedResults[categoryId][pilotId][batteryIndex].bestLap =
      normalizedTime || null;
  ...
  const openQualifyingBestLapModal = (
    ...
    const currentQualifyingBestLap =
      stageResults[categoryId]?.[pilotId]?.[batteryIndex]?.qualifyingBestLap ||
      "";
  ...
  const saveQualifyingBestLap = async () => {
    ...
    const { categoryId, pilotId, batteryIndex } =
      selectedPilotForQualifyingBestLap;
    ...
    if (
      ...
      !timePattern.test(qualifyingBestLapTimeInput.trim())
    ) {
      toast.error("Formato de tempo inválido. Use MM:SS.sss ou SS.sss");
      return;
    }
    ...
    updatedResults[categoryId][pilotId][batteryIndex].qualifyingBestLap =
      normalizedTime || null;
  ...
  const openTotalTimeModal = (
    ...
    const currentTotalTime =
      stageResults[categoryId]?.[pilotId]?.[batteryIndex]?.totalTime || "";
  ...
  const saveTotalTime = async () => {
    ...
    updatedResults[categoryId][pilotId][batteryIndex].totalTime =
      normalizedTime || null;
  ...
  const clearAllResults = async () => {
    ...
    try {
      ...
      const categoryPilots = registrations.filter(
        (reg) =>
          reg.categories.some(
            (rc: any) => rc.category.id === selectedOverviewCategory,
          ) &&
          stageParticipations.some(
            (part) =>
              part.userId === reg.userId &&
              part.categoryId === selectedOverviewCategory &&
              part.status === "confirmed",
          ),
      );
      ...
      categoryPilots.forEach((pilot) => {
        if (
          updatedResults[selectedOverviewCategory]?.[pilot.userId]?.[
            selectedBatteryIndex
          ]
        ) {
          // Manter apenas os karts sorteados, limpar todo o resto
          const kartAssignment =
            updatedResults[selectedOverviewCategory][pilot.userId][
              selectedBatteryIndex
            ].kart;
      ...
      try {
        await LapTimesService.deleteLapTimesByCategoryAndBattery(
          selectedStageId,
        ...
        setLapTimes((prev) => {
          ...
          if (updated[selectedOverviewCategory]) {
            ...
            updated[selectedOverviewCategory] = updated[
              selectedOverviewCategory
            ].filter(
      ...
      try {
        ...
        const penalties = getPenalties().filter(
          (penalty) =>
  ...
  const getFilteredPenalties = (categoryId: string, batteryIndex: number) => {
    return filteredPenalties.filter(
      (penalty) =>
  ...
  const getSortedPilots = (categoryPilots: any[], category: any) => {
    ...
    sorted.sort((a, b) => {
      const getValue = (pilot: any, col: string) => {
        const kart =
          fleetDrawResults[category.id]?.[pilot.userId]?.[selectedBatteryIndex]
            ?.kart || 0;
        const peso =
          stageResults[category.id]?.[pilot.userId]?.[selectedBatteryIndex]
            ?.weight;
        const start =
          stageResults[category.id]?.[pilot.userId]?.[selectedBatteryIndex]
            ?.startPosition || 0;
        ...
        switch (col) {
          ...
          case "totalTime":
            ...
            const [totalMinutes, totalSeconds] = totalTime.includes(":")
              ...
              : ["0", totalTime];
            return (
              parseFloat(totalMinutes) * 60000 + parseFloat(totalSeconds) * 1000
            );
          case "piloto":
            return nome;
          default:
  ...
  const processPenaltiesFromExcel = (
    data: any[],
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (
        firstCell.includes("PENALIZAÇÕES") ||
        firstCell.includes("PENALIZACOES")
      ) {
      ...
      if (
        firstCell.includes("OBSERVAÇÕES DA PROVA") ||
        firstCell.includes("OBSERVACOES DA PROVA")
      ) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (foundObservationsSectionForFlags) {
        ...
        const blackFlagPattern =
          /O\s+competidor\s+de\s+nº\s+(\d+)\s+recebeu\s+uma\s+bandeira\s+preta/i;
        ...
        const warningParts = rowText.split(
          /(?=O\s+competidor\s+de\s+nº\s+\d+\s+recebeu\s+uma\s+ADV)/i,
        );
        ...
        for (const part of warningParts) {
          ...
          const warningMatch = part.match(
            /O\s+competidor\s+de\s+nº\s+(\d+)\s+recebeu\s+uma\s+ADV[^.]*(?:SOB\s+KART\s+(\d+))?/i,
          );

          if (warningMatch) {
            const competitorNumber = parseInt(warningMatch[1]);
            const kartNumberFromText = warningMatch[2]
              ? parseInt(warningMatch[2])
              : null;
  ...
  const loadLapTimes = async (categoryId: string) => {
    try {
      ...
      const lapTimesData = await LapTimesService.getLapTimesByStageAndCategory(
        selectedStageId,
        categoryId,
      );
      setLapTimes((prev) => ({
  ...
  const processImportFile = async () => {
    ...
    if (!categoryResults || Object.keys(categoryResults).length === 0) {
      toast.error(
        "É necessário realizar o sorteio de karts antes de importar os resultados.",
      );
      return;
    }
      ...
      if (importType === "lapTimes") {
        ...
        try {
          ...
          if (result.imported > 0) {
            ...
            toast.error(
              "Nenhum tempo volta a volta foi importado. Verifique se o arquivo está no formato correto.",
            );
          }
        } catch (error) {
          console.error("Erro ao processar volta a volta:", error);
          toast.error(
            "Erro ao processar arquivo de volta a volta. Verifique o formato.",
          );
        }
      } else {
        ...
        for (const sheetName of workbook.SheetNames) {
          ...
          for (let i = headerRow + 1; i < data.length; i++) {
            ...
            const positionValue = String(row[positionColumn])
              .trim()
              .toUpperCase();
            const kartNumber = Number(row[kartColumn]);
            const bestLapValue =
              bestLapColumn >= 0 ? String(row[bestLapColumn] || "").trim() : "";
            const totalTimeValue =
              totalTimeColumn >= 0
                ? String(row[totalTimeColumn] || "").trim()
                : "";
            const totalLapsValue =
              totalLapsColumn >= 0
                ? String(row[totalLapsColumn] || "").trim()
                : "";
            ...
            for (const [pilotId, batteryResults] of Object.entries(
              categoryResults,
            )) {
              const pilotBatteryData = batteryResults[selectedBatteryIndex];
              if (pilotBatteryData && pilotBatteryData.kart === kartNumber) {
                const fieldToUpdate =
                  importType === "race" ? "finishPosition" : "startPosition";
          ...
          if (importType === "race") {
            try {
              const penalties = processPenaltiesFromExcel(
                data,
                kartToUserMapping,
              );
              ...
              const existingPenalties = contextPenalties.filter(
                (penalty) =>
                  penalty.stageId === selectedStageId &&
                  penalty.categoryId === selectedOverviewCategory &&
                  penalty.batteryIndex === selectedBatteryIndex,
              ...
              for (const penalty of penalties) {
                ...
                if (userId) {
                  ...
                  const existingPenalty = existingPenalties.find(
                    (existing) =>
                  ...
                  if (existingPenalty) {
                    console.log(
                      `Punição duplicada descartada para kart ${penalty.kartNumber}: ${penalty.penaltyText}`,
                    );
                    penaltiesSkipped++;
                    continue;
                  }
                  ...
                  try {
                    ...
                    if (
                      ...
                      penalty.timePenaltySeconds > 0
                    ) {
                      penaltyTimeByPilot[userId] =
                        (penaltyTimeByPilot[userId] || 0) +
                        penalty.timePenaltySeconds;
                      penaltyTimeApplied++;
                    }
                  } catch (error) {
                    console.error(
                      `Erro ao criar penalidade para kart ${penalty.kartNumber}:`,
                      error,
                    );
                  }
                }
              ...
              for (const [userId, totalPenaltySeconds] of Object.entries(
                penaltyTimeByPilot,
              )) {
                const currentResults =
                  stageResults[selectedOverviewCategory]?.[userId]?.[
                    selectedBatteryIndex
                  ];
        ...
        if (allNotFoundKarts.length > 0) {
          ...
          toast.warning(
            `Karts não encontrados no sorteio: ${uniqueNotFoundKarts.join(", ")}`,
          );
        }
        ...
        if (
          ...
          toast.error(
            "Nenhum resultado foi importado. Verifique se o arquivo está no formato correto.",
          );
        }
      }
  ...
  return (
    <div className="space-y-6">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                ...
                >
                  {selectedOverviewCategory
                    ? categories.find(
                        (cat) => cat.id === selectedOverviewCategory,
                      )?.name || "Visão geral"
                    : "Visão geral"}
                  <ChevronDown className="w-4 h-4 text-muted-foreground" />
                </Button>
              ...
              <DropdownMenuContent align="end" className="w-48">
                <DropdownMenuItem
                  onClick={() => setSelectedOverviewCategory(null)}
                >
                  Visão geral
                </DropdownMenuItem>
                <DropdownMenuSeparator />
          ...
          <div className="relative">
            <select
              ...
              onChange={(e) => {
                ...
                if (newSeasonStages.length > 0) {
                  setSelectedStageId(
                    getClosestStage(newSeasonStages) || newSeasonStages[0].id,
                  );
                } else {
                  setSelectedStageId("");
                }
          ...
          (() => {
            ...
            return (
              <div className="flex flex-col">
                ...
                >
                  Bateria{" "}
                  <span className="text-gray-400">({batteries.length})</span>
                </label>
                <div className="relative">
                  <select
                    id="battery-select"
                    ...
                    onChange={(e) =>
                      setSelectedBatteryIndex(Number(e.target.value))
                    }
                  >
                            ...
                            {confirmedPilots.length > 0 ? (
                              confirmedPilots
                                .sort((a, b) => {
                                  ...
                                  ).localeCompare(b.user?.name || b.userId);
                                })
                                .map((pilot) => {
                                  ...
                                  const hasKartAssignments =
                                    pilotKartAssignments &&
                                    Object.keys(pilotKartAssignments).length >
                                      0;
                                  return (
                                    <div
                                      key={pilot.id}
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              ...
              <div className="flex flex-col gap-4 mb-4 w-full">
                {fleets.map((fleet, idx) => (
                  <div
                    key={fleet.id}
                    className="bg-white border border-gray-200 rounded-lg shadow-sm p-4 min-w-0"
                  >
                    <div
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <input
                          ...
                          value={fleet.name}
                          onChange={(e) =>
                            handleFleetNameChange(fleet.id, e.target.value)
                          }
                        ...
                        <div className="space-y-3 mb-4">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="text-sm text-gray-500 whitespace-nowrap">
                              Karts ativos: {getActiveKartsCount(fleet.id)}
                            </span>
                          ...
                          <div className="flex items-center gap-2">
                            <Button
                              ...
                              disabled={
                                fleet.totalKarts <= getMinKartsRequired()
                              }
                              tabIndex={-1}
                            >
                            ...
                            <Button
                              ...
                              onClick={(e) => {
                                ...
                                handleFleetKartsChange(
                                  fleet.id,
                                  Math.min(99, fleet.totalKarts + 1),
                                );
                              }}
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                  <div>
                    <div className="text-xl font-bold text-gray-900 mb-1">
                      {category.name} {categoryPilots.length}/
                      {category.maxPilots}
                    </div>
                    <div className="text-sm text-gray-600">
                      Pilotos confirmados para a etapa
                    </div>
                  </div>
                  {selectedOverviewCategory && (
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem
                          onClick={() => openImportModal("qualification")}
                        >
                          <Upload className="w-4 h-4 mr-2" />
                          Importar Qualificação
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() => openImportModal("race")}
                        >
                          <Upload className="w-4 h-4 mr-2" />
                          Importar Corrida
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() => openImportModal("lapTimes")}
                        >
                          <Upload className="w-4 h-4 mr-2" />
              ...
              <div className="mb-6">
                <div className="text-sm font-semibold text-gray-700 mb-2">
                  {batteries[selectedBatteryIndex]?.name ||
                    `Bateria ${selectedBatteryIndex + 1}`}
                </div>
                              ...
                              <div className="flex flex-col">
                                ...
                                >
                                  {(() => {
                                    ...
                                    if (
                                      pilotKartAssignments &&
                                      pilotKartAssignments[selectedBatteryIndex]
                                    ) {
                                      return pilotKartAssignments[
                                        selectedBatteryIndex
                                      ].kart;
                                    }
                                    return "-";
                              ...
                              <div className="flex flex-col">
                                ...
                                <button
                                  className={`py-3 px-4 rounded-lg font-semibold text-base border transition-colors ${
                                    stageResults[category.id]?.[pilot.userId]?.[
                                      selectedBatteryIndex
                                    ]?.weight === false
                                  ...
                                  onClick={() => {
                                    const currentWeight =
                                      stageResults[category.id]?.[
                                        pilot.userId
                                      ]?.[selectedBatteryIndex]?.weight;
                                    const newWeight =
                                      currentWeight === false ? true : false;
                                    updatePilotResult(
                                      category.id,
                                      pilot.userId,
                                      selectedBatteryIndex,
                                    ...
                                    <div className="flex items-center justify-center gap-1">
                                      ...
                                      {(() => {
                                        ...
                                        const categoryResults =
                                          stageResults[category.id] || {};
                                        const allBestLaps = Object.values(
                                          categoryResults,
                                        )
                                          .map(
                                            (pilotData: any) =>
                                              pilotData?.[selectedBatteryIndex]
                                                ?.bestLap,
                                          )
                                          .filter(Boolean);
                                        ...
                                        const bestTime = allBestLaps.reduce(
                                          (best, current) => {
                                            return convertTimeToMs(current) <
                                              convertTimeToMs(best)
                                              ? current
                                              : best;
                                          },
                                        );
                                ...
                                >
                                  {(() => {
                                    const totalTime =
                                      stageResults[category.id]?.[
                                        pilot.userId
                                      ]?.[selectedBatteryIndex]?.totalTime;
                                    const penaltyTime =
                                      stageResults[category.id]?.[
                                        pilot.userId
                                      ]?.[selectedBatteryIndex]?.penaltyTime;
                                    ...
                                    const penaltySeconds = penaltyTime
                                      ? parseInt(penaltyTime)
                                      : undefined;

                                    // Verificar se há punições importadas para este piloto nesta bateria
                                    const pilotPenalties =
                                      contextPenalties.filter(
                                        (penalty) =>
                                    ...
                                    const hasImportedPenalties =
                                      pilotPenalties.length > 0;

                                    const formattedTime = formatTimeWithPenalty(
                                      totalTime,
                                      penaltySeconds,
                                      false,
                                      hasImportedPenalties,
                                    );
                      ...
                      {getSortedPilots(categoryPilots, category).map(
                        (pilot, i) => (
                          ...
                          >
                            ...
                            >
                              <span
                                className={`${stageResults[category.id]?.[pilot.userId]?.[selectedBatteryIndex]?.weight === false ? "text-orange-600" : "text-green-600"} text-xs font-medium truncate`}
                              >
                                {stageResults[category.id]?.[pilot.userId]?.[
                                  selectedBatteryIndex
                                ]?.weight === false
                            ...
                            >
                              {(() => {
                                ...
                                return stageResults[category.id]?.[
                                  pilot.userId
                                ]?.[selectedBatteryIndex]?.finishPosition ? (
                                  <span className="font-medium">
                                    {
                                      stageResults[category.id][pilot.userId][
                                        selectedBatteryIndex
                            ...
                            >
                              {(() => {
                                ...
                                // Verificar se há punições importadas para este piloto nesta bateria
                                const pilotPenalties = contextPenalties.filter(
                                  (penalty) =>
                                    penalty.userId === pilot.userId &&
                                    penalty.stageId === selectedStageId &&
                                    penalty.categoryId === category.id &&
              ...
              {(() => {
                    ...
                    <div className="mt-6">
                      ...
                      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                        <Loading
                          type="spinner"
                          size="sm"
                          message="Carregando penalidades..."
                        />
                      </div>
                    </div>
                  );
                ...
                const categoryPenalties = getFilteredPenalties(
                  category.id,
                  selectedBatteryIndex,
                );
                      ...
                      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                        <div className="text-center text-green-700">
                          ...
                          <p className="text-sm font-medium">
                            Nenhuma penalidade aplicada nesta bateria
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                return (
                  <div className="mt-6">
                    <div className="mb-4">
                      ...
                      >
                        {categoryPenalties.length} penalidade
                        {categoryPenalties.length !== 1 ? "s" : ""}
                      </Badge>
                    </div>
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex flex-wrap items-center gap-2 mb-2">
                                  ...
                                  <span className="font-semibold text-gray-900 break-words">
                                    {formatName(
                                      penalty.user?.name || penalty.userId,
                                    )}
                                  </span>
                                  <Badge
                                    className={`${getPenaltyStatusColor(penalty.status)} flex-shrink-0`}
                                  >
                                ...
                                <div className="flex flex-wrap gap-4 text-xs text-gray-500">
                                  {penalty.timePenaltySeconds && (
                                    <span className="flex items-center gap-1 flex-shrink-0">
                                      ...
                                      {penalty.timePenaltySeconds}s de
                                      penalidade
                                    </span>
                                  )}
                                  {penalty.positionPenalty && (
                                    <span className="flex items-center gap-1 flex-shrink-0">
                                      ...
                                      {penalty.positionPenalty} posição
                                      {penalty.positionPenalty !== 1
                                        ? "s"
                                        : ""}{" "}
                                      de penalidade
                                    </span>
            ...
            <div className="relative bg-white rounded-lg shadow-xl max-w-6xl max-h-[90vh] w-full mx-4 overflow-hidden flex flex-col z-10">
              ...
              <div className="flex items-center justify-between p-6 border-b border-gray-200">
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Configure a frota para cada categoria e realize o sorteio
                    dos karts por bateria.
                  </p>
                </div>
                <button
                  onClick={handleCloseFleetDrawModal}
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                {Object.entries(fleetDrawResults[category.id])
                                  .map(([pilotId, batteryResults]) => {
                                    const pilot = registrations.find(
                                      (reg) => reg.userId === pilotId,
                                    );
                                    return {
                                      pilotId,
                                      batteryResults,
                                      pilotName: formatName(
                                        pilot?.user?.name || pilotId,
                                      ),
                                    };
                                  })
                                  .sort((a, b) =>
                                    a.pilotName.localeCompare(
                                      b.pilotName,
                                      "pt-BR",
                                    ),
                                  )
                                    ...
                                    ({
                                      ...
                                      >
                                        ...
                                        <div className="text-xs text-gray-600 mt-1 space-y-1">
                                          {Object.entries(batteryResults).map(
                                            ([batteryIdx, result]) => (
                                              <div key={batteryIdx}>
                                                Bateria {Number(batteryIdx) + 1}
                                                : Kart {result.kart}
                                              </div>
                                            ),
                                          )}
              ...
              <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-200">
                <Button variant="outline" onClick={handleCloseFleetDrawModal}>
                  Fechar
                </Button>
                <Button
                  onClick={async () => {
      ...
      {showKartSelectionModal &&
        selectedPilotForKartChange &&
        createPortal(
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            {/* Backdrop */}
            <div
                ...
                <div className="space-y-4">
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <p className="text-sm text-gray-600 mb-4">
                      Selecione um kart da frota para a bateria{" "}
                      {selectedPilotForKartChange.batteryIndex + 1}. Karts em
                      preto estão sendo usados por outros pilotos e serão
                      ...
                      const categoryPilots = registrations.filter(
                        (reg) =>
                          reg.categories.some(
                            (rc: any) =>
                              rc.category.id ===
                              selectedPilotForPosition.categoryId,
                          ) &&
                          stageParticipations.some(
                            (part) =>
                              part.userId === reg.userId &&
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    ...
                    <ul className="text-sm text-blue-800 space-y-1">
                      <li>
                        • <strong>MM:SS.sss</strong> (ex: 1:23.456)
                      </li>
                      <li>
                        • <strong>SS.sss</strong> (ex: 47.123)
                      </li>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full mx-4 overflow-hidden flex flex-col z-10">
              ...
              <div className="flex items-center justify-between p-6 border-b border-gray-200">
                <div>
                  <h2 className="text-xl font-semibold text-gray-900">
                    Editar Melhor Volta de Qualificação
                  </h2>
                  <p className="text-sm text-gray-600 mt-1">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setQualifyingBestLapTimeInput(e.target.value)
                      }
                      placeholder="Ex: 1:23.456 ou 47.123"
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 font-mono"
                      disabled={qualifyingBestLapLoading}
              ...
              <div className="flex-1 p-6">
                <div className="space-y-4">
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    ...
                    <ul className="text-sm text-blue-800 space-y-1">
                      <li>
                        • <strong>MM:SS.sss</strong> (ex: 14:34.610)
                      </li>
                      <li>• Deixe em branco para remover o tempo</li>
                    </ul>
                  </div>
      ...
      >
        <DialogContent className="w-[95vw] max-w-2xl p-4 sm:p-6">
          ...
          <DialogFooter>
            <div className="flex gap-2">
              <Button variant="outline" onClick={handleCloseShareScheduleModal}>
                Fechar
              </Button>
              <Button
          ...
          {clearAllLoading ? (
            ...
            >
              <Loading
                type="spinner"
                size="md"
                message="Limpando dados... Este processo pode demorar alguns segundos."
              />
            </div>
          ) : (
            <>
