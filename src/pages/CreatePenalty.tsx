...
import { PenaltyType } from "../lib/services/penalty.service";
...
export const CreatePenalty = () => {
  ...
  const { championshipId, penaltyId } = useParams<{
      ...
      try {
        ...
        const baseConfig: FormSectionConfig[] = [
          {
            ...
            detail:
              "Selecione a temporada, etapa, categoria e piloto onde a punição será aplicada",
            fields: [
              {
                id: "seasonId",
                name: "Temporada",
            ...
            fields: [
              {
                ...
                options: [
                  {
                    value: PenaltyType.DISQUALIFICATION,
                    description: "Desqualificação",
                  },
                  {
                    value: PenaltyType.TIME_PENALTY,
                    description: "Penalidade de Tempo",
                  },
                  {
                    value: PenaltyType.POSITION_PENALTY,
                    description: "Penalidade de Posição",
                  },
        ...
        if (isEditMode && penaltyData) {
          setInitialValues({
            ...
            timePenaltySeconds: penaltyData.timePenaltySeconds
              ? String(penaltyData.timePenaltySeconds)
              : "",
            positionPenalty: penaltyData.positionPenalty
              ? String(penaltyData.positionPenalty)
              : "",
            batteryIndex:
              penaltyData.batteryIndex !== undefined
                ? String(penaltyData.batteryIndex)
                : "",
          });
          ...
          if (penaltyData.categoryId && penaltyData.seasonId) {
            await loadPilotsForCategory(
              penaltyData.categoryId,
              penaltyData.seasonId,
              penaltyData.stageId,
            );
          }
        } else {
  ...
  const loadStagesForSeason = useCallback(
    async (seasonId: string) => {
      ...
      try {
        ...
        const seasonStages = allStages.filter(
  ...
  const loadCategoriesForStage = useCallback(
  ...
  const loadPilotsForCategory = useCallback(
    async (categoryId: string, seasonId: string, stageId?: string) => {
      ...
      try {
        ...
        const registrationsRes = allRegistrations.filter(
          (reg) => reg.seasonId === seasonId,
        );
        ...
        const confirmedPilots = registrationsRes.filter(
          (reg: any) =>
  ...
  const transformSubmitData = useCallback(
    (data: any) => {
      ...
      if (
        data.type === PenaltyType.TIME_PENALTY &&
        (!data.timePenaltySeconds || data.timePenaltySeconds <= 0)
      ) {
        throw new Error("Informe o tempo de penalidade em segundos.");
      }
      if (
        data.type === PenaltyType.POSITION_PENALTY &&
        (!data.positionPenalty || data.positionPenalty <= 0)
      ) {
  ...
  const handleSubmit = useCallback(
    async (data: any) => {
      ...
      if (
        ...
        try {
          ...
          if (stage && stage.stage_results) {
            ...
            const prevPenalty = batteryResults.penaltyTime
            ...
            const updatedPilotResults = {
              ...pilotResults,
              [data.batteryIndex]: updatedBatteryResults,
            };
            const updatedCatResults = {
              ...catResults,
              [data.userId]: updatedPilotResults,
            };
            const updatedResults = {
              ...results,
              [data.categoryId]: updatedCatResults,
            };
            ...
            try {
              ...
              console.log(
                "🔄 [FRONTEND] Buscando dados atualizados da etapa...",
              );
              ...
              console.log(
                "🔄 [FRONTEND] Atualizando participações da etapa no contexto...",
              );
      ...
      if (
        data.type === PenaltyType.DISQUALIFICATION &&
        data.seasonId &&
        data.stageId &&
        data.categoryId &&
        data.userId &&
        ...
        try {
          ...
          try {
            console.log(
              "🔄 [FRONTEND] Iniciando recálculo de posições após desclassificação...",
            );
            await ChampionshipClassificationService.recalculateStagePositions(
              data.stageId,
              data.categoryId,
            ...
            console.log(
              "✅ Posições recalculadas com sucesso após desclassificação",
            );
            ...
            console.error(
              "❌ Erro ao recalcular posições após desclassificação:",
              recalcError,
            );
            // Não bloquear o sucesso da criação da punição se o recálculo falhar
          }
          ...
          console.error(
            "Erro ao atualizar contexto após desclassificação:",
            err,
          );
        }
      }
      ...
      if (
        ...
        try {
          ...
          try {
            console.log(
              "🔄 [FRONTEND] Iniciando recálculo de posições após penalidade de posição...",
            );
            await ChampionshipClassificationService.recalculateStagePositions(
              data.stageId,
              data.categoryId,
            ...
            console.log(
              "✅ Posições recalculadas com sucesso após penalidade de posição",
            );
            ...
            console.error(
              "❌ Erro ao recalcular posições após penalidade de posição:",
              recalcError,
            );
            // Não bloquear o sucesso da criação da punição se o recálculo falhar
          }
          ...
          console.error(
            "Erro ao atualizar contexto após penalidade de posição:",
            err,
          );
        }
      }
  ...
  useEffect(() => {
    if (selectedCategory && selectedCategory.batteriesConfig) {
      const batteryOptions = selectedCategory.batteriesConfig.map(
        (battery, index) => ({
          value: String(index),
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-destructive">Acesso Negado</h1>
          <p className="text-muted-foreground">
            Você não tem permissão para criar penalidades
          </p>
        </div>
      </div>
    );
  ...
  return (
    <FormScreen
      ...
      successMessage={
        isEditMode
          ? "Punição atualizada com sucesso!"
          : "Punição criada com sucesso!"
      }
      errorMessage={
        isEditMode ? "Erro ao atualizar punição." : "Erro ao criar punição."
      }
    />
  );
};
