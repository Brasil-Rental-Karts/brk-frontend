...
import {
  SeasonRegistration as UserRegistration,
...
export const SeasonRegistration: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const existingRegistration = userRegistrationsData.find(
          (reg) => reg.seasonId === seasonData.id,
        );

        // Obter condições de pagamento ativas
        const activeConditions =
          seasonData.paymentConditions?.filter((c) => c.enabled) || [];

        if (existingRegistration) {
          // Se o usuário já está inscrito, determinar o tipo baseado na inscrição existente
          const hasStages =
            existingRegistration.stages &&
            existingRegistration.stages.length > 0;
          ...
          const compatibleConditions = activeConditions.filter(
            (c) => c.type === registrationType,
          );
          ...
          if (activeConditions.length === 1) {
            ...
            conditionType &&
            activeConditions.some((c) => c.type === conditionType)
          ) {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro ao carregar dados da temporada";
  ...
  let activeConditions =
    season.paymentConditions?.filter((c) => c.enabled) || [];

  // Se o usuário já está inscrito, filtrar apenas condições compatíveis
  if (existingRegistration) {
    const hasStages =
      existingRegistration.stages && existingRegistration.stages.length > 0;
    const registrationType = hasStages ? "por_etapa" : "por_temporada";
        ...
        <div className="min-h-screen bg-background flex items-center justify-center">
          <div className="text-center">
            <h1 className="text-2xl font-bold text-destructive">
              Inscrição Incompatível
            </h1>
            <p className="text-muted-foreground">
              Você já está inscrito nesta temporada com o tipo "
              {registrationType === "por_temporada"
                ? "Por Temporada"
                : "Por Etapa"}
        ...
        <div className="max-w-4xl mx-auto">
          <Card>
            ...
            <CardContent className="p-6">
                ...
                <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="text-center">
                    ...
                    <p className="text-blue-700">
                      Como você já está inscrito com o tipo "
                      {existingRegistration.stages &&
                      existingRegistration.stages.length > 0
                        ? "Por Etapa"
                        : "Por Temporada"}
              ...
              <div
                className={`grid gap-6 ${activeConditions.length === 1 ? "grid-cols-1 max-w-md mx-auto" : "grid-cols-1 md:grid-cols-2"}`}
              >
                    ...
                    <div className="space-y-3">
                      ...
                      {condition.type === "por_etapa" && (
                        <div className="text-sm text-gray-600 bg-blue-50 p-2 rounded">
                          <strong>Por Etapa:</strong> Você escolherá quais
                          etapas participar e pagará apenas pelas etapas
                          selecionadas.
                        </div>
                      )}
                      ...
                      {condition.type === "por_temporada" && (
                        <div className="text-sm text-gray-600 bg-green-50 p-2 rounded">
                          <strong>Por Temporada:</strong> Você se inscreve na
                          temporada completa e pode participar de todas as
                          etapas.
                        </div>
                      )}
                    </div>
