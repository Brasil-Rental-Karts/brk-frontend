...
import { CreditCardFees, CreateCreditCardFeesData, UpdateCreditCardFeesData } from '../lib/services/credit-card-fees.service';
import { Button, Card, CardContent, CardDescription, CardHeader, CardTitle } from 'brk-design-system';
...
export default function CreditCardFeesAdmin() {
  const { fees, loading, error, fetchFees, createFee, updateFee, deleteFee } = useCreditCardFees();
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold">Configuração de Taxas do Cartão de Crédito</h1>
      ...
      <div className="mb-4">
        <label htmlFor="championshipFilter" className="block text-sm font-medium mb-2">
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            <div className="space-y-4">
              <div>
                <label htmlFor="championshipId" className="block text-sm font-medium mb-2">
                  Campeonato
                </label>
                <select
                  id="championshipId"
              ...
              <div>
                <label htmlFor="installmentRange" className="block text-sm font-medium mb-2">
                  Range de Parcelas
                </label>
                <input
                  id="installmentRange"
                  ...
                  onChange={(e) => setFormData(prev => ({ ...prev, installmentRange: e.target.value }))}
                  placeholder="Ex: 1, 2-6, 7-12, 13-21"
                  className="w-full p-2 border border-gray-300 rounded-md"
                />
              </div>
              <div>
                <label htmlFor="percentageRate" className="block text-sm font-medium mb-2">
                  Taxa Percentual (%)
                </label>
                <input
                  id="percentageRate"
              ...
              <div>
                <label htmlFor="fixedFee" className="block text-sm font-medium mb-2">
                  Taxa Fixa (R$)
                </label>
                <input
                  id="fixedFee"
      ...
      {showEditForm && editingFee && (
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>
              Atualize as configurações da taxa
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div>
                <label htmlFor="editInstallmentRange" className="block text-sm font-medium mb-2">
                  Range de Parcelas
                </label>
                <input
              ...
              <div>
                <label htmlFor="editPercentageRate" className="block text-sm font-medium mb-2">
                  Taxa Percentual (%)
                </label>
                <input
              ...
              <div>
                <label htmlFor="editFixedFee" className="block text-sm font-medium mb-2">
                  Taxa Fixa (R$)
                </label>
                <input
                  id="editFixedFee"
              ...
              <div>
                <label htmlFor="editDescription" className="block text-sm font-medium mb-2">
                  Descrição
                </label>
                <input
                  id="editDescription"
            ...
            <Card key={fee.id}>
              <CardHeader>
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex items-center gap-2">
                    <Button variant="outline" size="sm" onClick={() => openEditForm(fee)}>
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium">Range de Parcelas</label>
                    <p className="text-sm text-muted-foreground">{fee.installmentRange}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium">Taxa Percentual</label>
                    <p className="text-sm text-muted-foreground">{toNumber(fee.percentageRate).toFixed(2)}%</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium">Taxa Fixa</label>
                    <p className="text-sm text-muted-foreground">R$ {toNumber(fee.fixedFee).toFixed(2)}</p>
                  </div>
                  <div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full border border-gray-200 rounded-lg">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-700">Campeonato</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-700">Range de Parcelas</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-700">Taxa Percentual</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-700">Taxa Fixa</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-700">Descrição</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-700">Status</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-700">Ações</th>
              </tr>
            </thead>
            <tbody>
              {sortedFees.map((fee) => (
                <tr key={fee.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="px-4 py-2 whitespace-nowrap">{getChampionshipName(fee.championshipId)}</td>
                  <td className="px-4 py-2 whitespace-nowrap">{fee.installmentRange}</td>
                  <td className="px-4 py-2 whitespace-nowrap">{toNumber(fee.percentageRate).toFixed(2)}%</td>
                  <td className="px-4 py-2 whitespace-nowrap">R$ {toNumber(fee.fixedFee).toFixed(2)}</td>
                  <td className="px-4 py-2 whitespace-nowrap">{fee.description || '-'}</td>
                  <td className="px-4 py-2 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs rounded-full ${fee.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{fee.isActive ? 'Ativo' : 'Inativo'}</span>
                  </td>
                  <td className="px-4 py-2 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="sm" onClick={() => openEditForm(fee)}>
